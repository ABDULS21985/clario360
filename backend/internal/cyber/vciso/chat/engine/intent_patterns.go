	...
	return compileIntentPatterns([]intentSpec{
		{
			...
			PatternTexts: []string{`(?i)(what|show|tell|get).*(risk|security)\s*(score|posture|status|level|rating)`, `(?i)how\s*(secure|safe|risky|vulnerable)\s*(are we|is our|is the)`, `(?i)(risk|security)\s*(summary|assessment)`, `(?i)(current|overall|org|our).*(risk|security).*(score|status|level)`, `(?i)(what'?s|whats)\s*(the|our)\s*(risk|security)`},
		...
		},
		{
			...
			RequiresEntity: true,
			EntityType:     "alert_id",
			PatternTexts:   []string{`(?i)(show|tell|get|details?|info|about).*(alert|incident)\s*` + alertIDLike, `(?i)(what happened|explain|describe).*(alert|incident)\s*` + alertIDLike, `(?i)alert\s*` + alertIDLike, `(?i)\binvestigat(e|ion)\b.*\b(alert|incident)\b.*\b(details?|info|about)\b`, `(?i)(show|tell|get).*(details?|info|about).*(first|second|third|last|it|that|this)$`},
		...
		},
		{
			...
			PatternTexts:   []string{`(?i)(show|tell|get|find|look\s*up).*(about|detail|info).*(asset|server|host|machine|device|endpoint)`, `(?i)(what do we know about|show details?\s+for|details?\s*(for|on|about))\s+(?:\d{1,3}(?:\.\d{1,3}){3}|[\w.-]+)`, `(?i)(asset|server|host|device)\s+[\w.-]+\s*(info|detail|status|vuln)`, `(?i)(show|tell|get|find|look\s*up).*(web|db|host|srv|prod|dev|corp)[\w.-]*`},
		...
		},
		{
			...
			PatternTexts: []string{`(?i)(risky|suspicious|anomal|unusual).*(user|entity|account|employee|person)s?`, `(?i)(who|which).*(riskiest|risky|suspicious|anomal|unusual).*(user|entity|person|account)`, `(?i)(insider|behavioral).*(threat|risk|anomal)`, `(?i)ueba.*(summary|alert|risk|top)`, `(?i)(suspicious|anomalous)\s+activity`},
		...
		},
		{
			...
			PatternTexts: []string{`(?i)\b(what should|recommend|suggest|prioriti[sz]e|focus|action|todo)\b`, `(?i)\b(top|most important|urgent|critical)\b.*\b(action|task|priorit(?:y|ies)|thing|issue)s?\b`, `(?i)\b(what|where)\b.*\b(should|do|focus|start)\b.*\b(first|today|now|next)\b`, `(?i)\b(daily|morning|today)\b.*\b(brief|summary|priority|action)\b`, `(?i)\bwhat\b.*\b(matters?|urgent|important|press)\b.*\b(today|now|right now)\b`},
		...
		},
		{
			...
			RequiresEntity: true,
			EntityType:     "alert_id",
			PatternTexts:   []string{`(?i)investigat.*(alert|incident|event|threat)\s*` + alertIDLike, `(?i)(deep dive|analy[sz]e|look into|dig into|examine).*(alert|incident)\s*` + alertIDLike, `(?i)(full|complete|detailed).*(investigation|analysis).*(alert|incident)\s*` + alertIDLike, `(?i)investigat(e|ion).*(it|that|this|first|second|third|last)`},
		...
		},
		{
			...
			PatternTexts: []string{`(?i)(how|has).*(risk|alert|score|posture|threat).*(chang|trend|evolv|compar|improv|worsen)`, `(?i)(trend|change|history|comparison|progress).*(risk|alert|score|threat|security)`, `(?i)(alert|risk|threat).*(volume|count|score).*(trend|change|history)`, `(?i)(over time|week over week|month over month|getting better|getting worse)`, `(?i)(compar).*(this week|last week|this month|last month|yesterday)`},
		...
		},
		{
			...
			RequiresEntity: true,
			EntityType:     "alert_id",
			PatternTexts:   []string{`(?i)(start|run|execute|trigger|initiate|begin).*(remediat|fix|patch|mitiga|resolv).*(alert|vuln|issue|threat|incident)?\s*` + alertIDLike + `?`, `(?i)remediat.*(alert|vuln|issue|threat|incident)\s*` + alertIDLike, `(?i)(fix|patch|block|isolate|contain).*(alert|threat|vuln)\s*` + alertIDLike},
