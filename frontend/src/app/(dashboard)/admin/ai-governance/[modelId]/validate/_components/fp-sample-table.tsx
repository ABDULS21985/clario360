...
interface FPSampleTableProps {
  samples: AIValidationPredictionSample[];
}

export function FPSampleTable({ samples }: FPSampleTableProps) {
  const [expandedRow, setExpandedRow] = useState<string | null>(null);

  return (
    <Card className="border-border/70">
      <CardHeader>
        <CardTitle>False Positive Samples</CardTitle>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
            ...
            {samples.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} className="text-sm text-muted-foreground">
                  No false positives were captured in this validation run.
                </TableCell>
              </TableRow>
            ) : null}
            {samples.map((sample) => {
