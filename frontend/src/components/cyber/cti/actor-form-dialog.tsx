...
export function ActorFormDialog({
  ...
  const mutation = useMutation({
    mutationFn: async (values: ActorFormValues) => {
      if (actor) {
        return updateThreatActor(actor.id, {
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <FormField name="origin_country_code" label="Origin Country">
                <Input id="origin_country_code" placeholder="RU" maxLength={2} {...form.register('origin_country_code')} />
              </FormField>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={mutation.isPending}>
                {mutation.isPending ? 'Saving...' : actor ? 'Save Changes' : 'Create Actor'}
              </Button>
            </DialogFooter>
          </form>
        </FormProvider>
