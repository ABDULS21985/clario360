...
class BaseResource(Generic[T]):
    ...
    def _get_at(self, path: str, model: Type[T]) -> T:
        payload = self._http.get(path)
        return self._parse_model(payload, model)

    async def _aget_at(self, path: str, model: Type[T]) -> T:
        payload = await self._async_http.get(path)
        return self._parse_model(payload, model)

    def _post_at(self, path: str, model: Type[T], data: Optional[Mapping[str, Any]] = None) -> T:
    ...
    def _put_at(self, path: str, model: Type[T], data: Optional[Mapping[str, Any]] = None) -> T:
    ...
    def _list_models_at(self, path: str, model: Type[T]) -> List[T]:
        payload = self._http.get(path)
    ...
    def _parse_model(self, payload: Any, model: Type[T]) -> T:
    ...
    def _unwrap(self, payload: Any) -> Any:
        if isinstance(payload, dict) and "data" in payload and not ("access_token" in payload):
            return payload["data"]
        return payload
